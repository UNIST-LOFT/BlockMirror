...
def generate_code(description):
    try:
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are an assistant that writes Javascript code."},
                {"role": "user", "content": f"Please complete Javascript code just send it for the following task: {description}. Also, if there is a return value, please make sure to save that value in a variable named 'result' and then return that 'result' value."}
            ],
            temperature=0
        )
...
def create_function_block_xml(code):
    ...
    for func in function_defs:
        ...
        block.set("type", "ast_FunctionDef")
        ...
        body_block.set("type", "ast_ReturnFull")
